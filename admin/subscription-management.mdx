...
Returns a paginated list of all subscriptions, each loaded with the associated user (id, name, email). The response also enriches each record with the most recent related payment's `amount`, `comprobante`, and `status` to give the reviewing admin full context without opening a separate payment page.
