---
title: "Exams, Scoring, and Certificate Generation"
...
<Steps>
  ...
  <Step title="View the result">
    ...
    On failure, `status` is `"reprobado"` and `certificate_url` is `null`.
  </Step>
...
|----------|---------|-------------|
| `EDUCATION_PASSING_SCORE` | `14` | Minimum score (out of 20) required to pass; also used as `minimum_score` fallback on each `CourseQuiz` record |
...
The per-quiz `minimum_score` and `max_attempts` columns take precedence over these global defaults when set.
